...
of the SDSS Stripe 82 Standard Stars Catalog with Gaia, and Comparison with Pan-STARRS1, DES, 
...
Gaia DR2 (which are gone in Gaia EDR3). Due to its large size and cross-checks with other surveys, 
this updated SDSS catalog can be used to robustly calibrate or test $ugriz$ photometry below 
1\% level, for example, as needed for the commissioning phase of the Rubin Observatory Legacy 
...
For our primary astrometric and photometric cross calibration we use the Gaia data release DR2 catalogs.  Gaia is a European /Space Agency (ESA) mission designed to map over a billion stars in the Milky Way and the local group in three dimensions, providing accurate proper motion and radial velocity measurements. \citet{GaiaCollab2016} provide a detailed overview about the Gaia mission (spacecraft, instruments, survey and measurement principles, and operations), while technical details for specific topics relevant to our work may found in the following citations: pre-processing and source-list creation \citep{Fabr2016}, astrometric solution \citep{LInd2018}, processing the photometric data \citep{Riel2018}, photometric content and validation \citep{Evan2018}, and full catalog validation \citep{Aren2018}. A full description of the data products in Gaia DR2 may be found in \citet{GaiaCollab2018b}. Here we summarize only the pertinent details.

The second Gaia Data Release, Gaia DR2, includes astrometry, photometry, radial velocities, and information on astrophysical parameters and variability, for approximately 1.7 billion sources \citep{GaiaCollab2018b}. This dataset is based on the first 22 months of the mission and includes celestial positions and the apparent brightness in the broad-band G (Gmag hereafter) for sources brighter than Gmag$\sim$21.  This data release also contains two additional broad-band magnitudes, the BP (330-680 nm) and RP (630-1050 nm). Gaia DR2 photometry is generally superior to ground-based photometry for sources with sufficient signal-to-noise ratio, and we use it to derive zeropoint corrections for SDSS photometry, as described in Section~\ref{sec:GaiaCorr}. 

Just before this manuscript was ready for submission, Gaia Early Data Release 3 (EDR3)
became available. As we discuss in \S~\ref{sec:EDR3} below, the small photometric differences 
between DR2 and EDR3 are inconsequential for the results presented here. 
...
Note also large ($\sim0.03$ mag) discrepancy at the faint end -- a comparison 
of the SDSS catalog with Pan-STARRS and DES catalogs (see Figure~\ref{fig:drVSr}) 
suggests that its origin is a bias in Gaia's DR2 photometry at the faint end, rather than 
a problem with SDSS photometry. This bias is much smaller when using Gaia EDR3 
photometry (see Figure~\ref{fig:gaiaJumpEDR3}). }
\label{fig:gaiaJump}
\end{figure}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Comparison with Gaia EDR3 catalog \label{sec:EDR3}} 

The Gaia EDR3 catalog became available as this manuscript was ready for submission.
Although reported photometric differences between Gaia DR2 and EDR3 are too small
to have a substantial impact on the results presented here, we rederived gray zeropoint
correction for the v3.4 catalog using the EDR3 Gmag data. As expected, the implied 
zeropoint corrections show only tiny deviations from zero with an rms of only about 
2.2 milimag in R.A direction and 0.9 milimag in Dec direction. We also analyzed the 
changes of the BP-RP color by directly comparing DR2 and EDR3 values. The implied 
zeropoint changes have an rms of 3.1 milimag for R.A. direction and 0.8 milimag
for Dec direction, implying inconsequential changes for the zeropoint corrections
in the $ugiz$ bands.   

Nevertheless, there is one substantial change between DR2 and EDR3 photometry. 
We reported in Figure~\ref{fig:gaiaJump} a likely bias in Gaia DR2 Gmag photometry 
at the faint end of about 0.03 mag. As Figure~\ref{fig:gaiaJumpEDR3} shows, 
this bias is much smaller in EDR3. Furthermore, a jump by about 3 millimag 
at Gmag$\sim$16 is gone in EDR3. 
 

\begin{figure}[th!]
    \centering\includegraphics[width=9cm]{figures/GmagCorrectionTestEDR3_Gmag_Hess.png} 
\caption{Analogous to Figure~\ref{fig:gaiaJump}, except that here synthetic Gmag values 
generated using SDSS $gri$ photometry are compared to Gaia's Early Data Release 3 
instead of Data Release 2. This figure shows better agreement: the implied bright- to faint-end 
zeropoint gradient went from about 0.035 mag to $<$0.020 mag, and the small $\sim$milimag 
jump at Gmag$\sim$16 disappeared.}
\label{fig:gaiaJumpEDR3}
\end{figure}
...
\begin{table}
	...
	\begin{tabular}{l|c|c} % 
       ...
       $z$        &        8.9    &      8.4      \\ 
		\hline
	\end{tabular}
	\caption{The robust standard deviation for magnitude differences between the v2.6 (old)and v3.4 (new) catalogs. Notes: (a) For the $u$ band, the scatter in R.A. direction is due to more observations in v3.4 than in v2.6, rather than zeropoint correction.}
	\label{tab:oldnewRMS}
\end{table}
...
differences are shown as a function of the $r$ band magnitude. The scatter of median
...
of up to 0.01-0.02 mag, as a function of R.A. (see the bottom left panel in Figure~\ref{fig:DESPSRA}), although a similar trend in the equivalent Pan-STARRS plot (bottom right panel in Figure~\ref{fig:DESPSRA}) may indicate that the reason for this issue may reside partially with the Gaia DR2 calibration upon which the SDSS Stripe 82 calibrations are based \footnote{For a comparison of DES DR1 and Gaia DR2 calibrations, see e.g., Fig. 9 of \citet{2018ApJS..239...18A}}
...
0.02 mag, and thus demonstrates a likely problem with Gaia DR2 photometry. Note that 
this problem seems fixed in Gaia EDR3 photometry (see \S~\ref{sec:EDR3}), which strengthens
our conclusion. 
...
the new v3.4 SDSS catalog, and DES and Pan-STARRS1 (PS1) catalogs (millimag).}
	\label{tab:DESPS1}
\end{table}
...
faint end ($20<r<21$), where Gaia Gmag magnitudes from DR2 appear too faint by about
0.03 mag (see Figure~\ref{fig:gaiaJump}). This bias is much smaller when using Gaia EDR3 
photometry (see Figure~\ref{fig:gaiaJumpEDR3}). } 
\label{fig:drVSr}
\end{figure}
...
\begin{equation}
\begin{split}
u_{est} = & NUV  - 0.999\times(NUV - g) + 0.015\times(NUV  - g)^2  \\
         & -0.426\times(g - i) + 0.821*(g - i)^2 + 0.939 \\
         & + T \times [ -1.233\times(E(B-V)-0.15) ... \\
         & + 3.430\times(E(B-V)-0.15)^{2} ] \\
\end{split}
\end{equation}
...
1.0$, and $E(B-V) < 0.3$, has a per star RMS of $\sigma=0.060$ mag
...
magnitudes in to very reasonable approximations of the SDSS $u$-band.
To refine these approximations, we then applied the same sort of
numerical transformation that was used for the SDSS/Gaia
transfrormation in Figure~\ref{fig:GrVSgi}; this improved the overall
per star RMS to $\sigma=0.050$, with most of the improvement occurring
...
based upon the binned data consisting of 54,660 matches between the
v3.4 SDSS catalog and the GALEX catalog.  First, in
Figure~\ref{fig:GALEX_umag}, we show the difference between the
SDSS-measured and GALEX-predicted $u_{sdss}$ vs. SDSS $u_{sdss}$.
Although the relation, even when binned like here, is noisy, no
obvious strong trends are seen in this plot.  Second, in
Figure~\ref{fig:GALEX_RA}, we plot this magnitude difference vs. RA
along SDSS Stripe 82.  Here, a noticeable large-scale trends can
...
Fig.~\ref{fig:DESPSRA}). {\bf (DLT: Check this!)}.  Finally, in
Figure~\ref{fig:GALEX_Dec}, we plot this magnitude difference vs. DEC
across SDSS Stripe82.  Unlike for the CFIS comparison in the previous
section (see, e.g., Fig.~\ref{fig:CFIS}, the GALEX comparison covers
the full DEC range of Stripe 82.  Overall, there appear to be no
obvious large-scale trends.  The hints of smaller scale
($\sim0.4^{\circ}$) variations -- roughly on the scale of a SDSS
camera column -- seen in the CFIS comparison (Fig.~\ref{fig:CFIS}),
however, are not seen here, likely due to the relative ``noisiness''
of the GALEX comparison.


\begin{figure}[th!]
    \centering\includegraphics[width=9cm]{figures/colorResidGALEXug_du_umag_Hess.png}
\caption{Analogous to Figure~\ref{fig:gaiaJump}, except that here residuals 
  residuals between the SDSS $u$ band magnitudes and predicted $u$
  band magnitudes transformed from the GALEX catalog are shown. The
  binned median scatter is Z.Z millimag.
  {\bf (DLT:  replace with jupyternotebook-generate figure when available.)}
}
...
\begin{figure}[th!]
    \centering\includegraphics[width=9cm]{figures/colorResidGALEXug_du_RA_Hess.png}
\caption{Analogous to Figure~\ref{fig:graycorrRA}, except that here residuals 
  residuals between the SDSS $u$ band magnitudes and predicted $u$
  band magnitudes transformed from the GALEX catalog are shown.  The
  binned median scatter is Z.Z millimag.
  {\bf (DLT:  replace with jupyternotebook-generate figure when available.)}
...
\begin{figure}[th!]
    \centering\includegraphics[width=9cm]{figures/colorResidGALEXug_du_Dec_Hess.png}
\caption{Analogous to Figure~\ref{fig:graycorrDec}, except that here residuals 
  residuals between the SDSS $u$ band magnitudes and predicted $u$
  band magnitudes transformed from the GALEX catalog are shown.  The
  binned median scatter is Z.Z millimag.
...
% uncertainties for the synthetic magnitudes of the 2 DAâ€™s from their sample at about 0.5% (5 milli-mags), but I donâ€™t know if 
...
% usually get a spread of less than 1% in the resulting synthetic mags; so I think Gautham Narayanâ€™s estimate of a statistical
...
{bf Gaia DR2}: This work has made use of data from the European Space Agency (ESA) mission {\it Gaia} (\url{https://www.cosmos.esa.int/gaia}), processed by the {\it Gaia} Data Processing and Analysis Consortium (DPAC,\url{https://www.cosmos.esa.int/web/gaia/dpac/consortium}). Funding for the DPAChas been provided by national institutions, in particular the institutions participating in the {\it Gaia} Multilateral Agreement.\\
