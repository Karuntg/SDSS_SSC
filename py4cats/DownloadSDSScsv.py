...
## objlist = numpy.loadtxt('S82_3',comments='#',delimiter=',')
rootname = sys.argv[1]
infile = sys.argv[2]
objlist = numpy.loadtxt(infile,comments='#',delimiter=',')
downloadJPG = 0
print objlist
...
for line in objlist:
    ...
    for camcol in range(1,7):
        ...
        for iField in range(minField,maxField+1):
            ...
            numberFields += 1                
            success += dumpSDSSfile(outdir, run, camcol, iField, downloadJPG)
	    ...
	    if chk4FitsTable(indir,infits):
		    ...
		    print "Badfits %s %s" % (indir,infits)
