...
def convFits2CSV(indir,infits):
		...
		f1.write("## obj_ctr,RA,Dec,MJD,objc_type,objc_rowc,objc_colc,nchild,rowc[5],rowcerr[5],colc[5],colcerr[5],psfmag[5],psfmagerr[5],modelmag[5],modelmagerr[5],tai[5],airmass[5],psf_fwhm[5],skyflux[5]\n")

		j = 0
		while j <= nobj-1:
## while j <= 1:
			outstr = str(j+1)+','+str(ra[j])+','+str(dec[j])+','+str(mjd[j])+','+str(objc_type[j])+','+str(objc_rowc[j])+','+str(objc_colc[j])+','+str(nchild[j])

			## make rowc string
			rowcstr = ''
			for tmprowc in rowc[j][:]:
				tmpstr = ','+str(tmprowc)
				rowcstr += tmpstr

			## make rowcerr string
			rowcerrstr = ''
			for tmprowcerr in rowcerr[j][:]:
				tmpstr = ','+str(tmprowcerr)
				rowcerrstr += tmpstr

			## make colc string
			colcstr = ''
			for tmpcolc in colc[j][:]:
				tmpstr = ','+str(tmpcolc)
				colcstr += tmpstr

			## make colcerr string
			colcerrstr = ''
			for tmpcolcerr in colcerr[j][:]:
			...
			modstr = ''
			for tmpmod in modelmag[j][:]:
				tmpstr = ','+str(tmpmod)
				modstr += tmpstr

			## make psf mag err string
			moderrstr = ''
			for tmpmoderr in modelmagerr[j][:]:
			...
			outstr += rowcstr+rowcerrstr+colcstr+colcerrstr+magstr+errstr+modstr+moderrstr+taistr+amstr+fwstr+sfstr
			f1.write(outstr+'\n')
