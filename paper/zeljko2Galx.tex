...
we do not attempt to derive analytic fits for {\bf (DLT:) synthetic} magnitudes and colors but instead
...
{\bf (DLT:), although a similar trend in the equivalent Pan-STARRS plot (bottom right panel in Figure~\ref{fig:DESPSRA})
may indicate some of the issue may also reside with the Gaia DR2 calibrations upon which the SDSS Stripee82 calibrations are based.\footnote{
For a comparison of DES DR1 and Gaia DR2 calibrations, see, e.g., Fig.~9 of \citet{2018ApJS..239...18A}.}}
...
A star-by-star comparison for about 150,000 sufficiently bright stars ($r<20$), with colors matching the main sequence ($1.0 <u-g < 2.1$)  is illustrated in Figure~\ref{fig:CFIS}. The binned median scatter for Declination direction is 
...
\begin{equation}
\begin{split}
u_{est} = & NUV_{galex}  - 0.999\times(NUV_{galex} - g_{sdss}) + 0.015\times(NUV_{galex}  - g_{sdss})^2  \\
         & -0.426\times(g_{sdss} - i_{sdss}) + 0.821*(g_{sdss} - i_{sdss})^2 + 0.939 \\
         & + T \times [ -1.233\times(E(B-V)-0.15) + 3.430\times(E(B-V)-0.15)^{2} ] \\
\end{split}
\end{equation}
where $T$ is a step function in the value of the interstellar reddening, E(B-V),
...
obvious strong trends are in seen in this plot.  Second, in
...
($\sim0.4^{\circ}$) variations -- roughly on the scale of an SDSS
cameral column -- seen in the CFIS comparison (Fig.~\ref{fig:CFIS}),
...
% u_{est} =  NUV_{galex} + 0.939 - 1.008*(NUV_{galex} - g_{sdss}) + 0.017*(NUV_{galex} - g_{sdss})^2 \\
%                    - 0.599*(g_{sdss} - i_{sdss}) + 0.860*(g_{sdss} - i_{sdss})^2

%   u_{est} =  NUV + 0.939 - 1.008*(NUV - g) + 0.017*(NUV - g)^2 - 0.599*(g - i) + 0.860*(g - i)^2
