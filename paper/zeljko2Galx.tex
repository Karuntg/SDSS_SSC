...
So, Sahar had matched GALEX against SDSS DR13 a while back, and we had generated initial GALEX -> SDSS u transformation equations.  It turns out using GALEX NUV with SDSS (g,i) or with SDSS (r,i) yields the tightest GALEX->SDSS u transformation equations we could estimate.  In what follows, we use this equation:
 
 \begin{equation}
  u_sdss = NUV_galex
           + 0.939
           ? 1.008*(NUV_galex ? g_sdss) + 0.017*(NUV_galex ? g_sdss)**2
           ? 0.599*(g_sdss ? i_sdss) + 0.860*(g_sdss ? i_sdss)**2
 \end{equation}
 
The rms per star for this relation is 0.071 mag ? so still a bit noisy, but about as good as we think we can get for transforming GALEX to SDSS u (but see below regarding E(B-V)?).
