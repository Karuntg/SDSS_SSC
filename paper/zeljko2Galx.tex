...
\subsubsection{Gaia-based photometric zeropoint corrections  \label{sec:GaiaCorr}}
...
Quality plots for this fit are shown in Figure \ref{figures/GalexQA.png} {\bf need to redo these plots in BW; select subplots to include}. The rms per star for this relation is 0.071 mag ? so still a bit noisy, but about as good as we think we can get for transforming GALEX to SDSS u (but see below regarding E(B-V)?).
