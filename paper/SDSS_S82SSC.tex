...
\newcommand{\KT}[1]{\textcolor{magenta}{#1}} % Karun
\newcommand{\ZI}[1]{\textcolor{blue}{#1}} % Zeljko
\newcommand{\AS}[1]{\textcolor{purple}{#1}} % Allyn
\newcommand{\DT}[1]{\textcolor{green}{#1}} % Douglas
%%%%%%%%%%%%%%%%%
...
Gaia DR2 photometry is {\bf (DLT:) generally} superior to ground-based
photometry for sources with sufficient signal-to-noise ratio, and we
use it to derive zeropoint corrections for SDSS photometry, as
described in Section~\ref{sec:v34}.
...
Our matched GALEX catalog consists of 150945 NUV sources in the S82 footprint for which we obtained NUV aperture magnitudes and uncertainties, as well as various fitted geometric measures for each of them. The astrometry has greater uncertainty than that of SDSS, with mean positional uncertainties of {\bf (DLT:) 1.0'' rms; we also note that there is an overall mean shift in the GALEX astrometry relative to tha of the SDSS by -0.1'' (RA) and +0.3'' (DEC).}   In addition, we find that the available depth in the S82 region is not very deep, with $>$80\% of the matched sample brighter than r=18 mag. 
